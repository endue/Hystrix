...
/* package */abstract class AbstractCommand<R> implements HystrixInvokableInfo<R>, HystrixObservable<R> {
    ...
    public Observable<R> toObservable() {
        ...
        return Observable.defer(new Func0<Observable<R>>() {
            ...
            @Override
            public Observable<R> call() {
                 /* this is a stateful object so can only be used once. command是有状态对象，所以只能使用一次 */
                ...
                if (requestCacheEnabled) {
                    // 从缓存中获取响应不为null调用handleRequestCacheHitAndEmitValues()
                ...
                return afterCache
                        .doOnTerminate(terminateCommandCleanup)     // 订阅即将被终止时的监听，无论是正常终止还是异常终止 perform cleanup once (either on normal terminal state (this line), or unsubscribe (next line))
                        .doOnUnsubscribe(unsubscribeCommandCleanup) // 取消订阅时的监听 perform cleanup once
                        .doOnCompleted(fireOnCompletedHook);        // 正常结束时的监听
            }
        });
    }
    ...
    private Observable<R> applyHystrixSemantics(final AbstractCommand<R> _cmd) {
        ...
        /* determine if we're allowed to execute 确定请求是否允许执行 */
        // (1)、熔断器未熔断
    ...
    /**
     * 获取隔离策略
     * 1. 信号量隔离策略，则根据commandKey获取信号量，不存在时初始化并缓存。
     * 控制单个服务调用并发度，采用请求线程本身执行请求(相对于线程池隔离策略)，从而也就没有上下文切换的开销，但是超时机制失效
     * 2. 线程池隔离策略，返回TryableSemaphoreNoOp.DEFAULT，也就是全部请求可以通过
